...
##注意jupyterhub_config.py要增加下面的配置
...
helm upgrade --cleanup-on-fail \
  --install jhub ./jupyterhub-1.1.3.tgz \
  --namespace jhub \
  --create-namespace \
  --values jupyterhub-config.yaml \
  --set-file hub.extraFiles.my_config.stringData=jupyterhub_config.py \
  --set-file hub.extraFiles.login.stringData=login.html