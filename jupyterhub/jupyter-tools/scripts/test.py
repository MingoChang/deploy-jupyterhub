...
''')
    ...
    parent_parser.add_argument('-c', '--count', default=100, type=int,
                               help='Number of users/servers (pods) to create '
                                    '(default: 100).')
    parent_parser.add_argument("-s", "--start", default=0, type=int, help='start index')
    parent_parser.add_argument("-e", "--end", default=0, type=int, help='end index')
    ...
    experiment_parser = subparsers.add_parser(
        'experiment', parents=[parent_parser]
    )

    experiment_parser.add_argument(
        '--cmd', type=str, default='bash tar ',
        help='end '
    )
    experiment_parser.add_argument(
        '--prefix', type=str, default='jupyter-hub-2dstress-2dtest-2d',
        help='end '
    )
    experiment_parser.add_argument(
        '--workers', type=int, default=100,
...
def find_existing_stress_test_users(endpoint, session):
    ...
    if resp:
        ...
        if users:
            users = list(
                filter(lambda user: user['name'].startswith('sd00'),
                       users))
...
@timeit
def experiment(prefix, startindex, endindex, cmd, workers):
    @timeit
    def activity(name):
        try:
            LOG.info("开始做实验%s" % name)
            subprocess.run(
                    'kubectl exec -it {0} -n jhub -- tar -zcf /home/jovyan/{1}.tar.gz /home/jovyan/com_rec'.format(
                        name,name),shell=True)
            p = subprocess.run(
                'kubectl cp  -n jhub {0}:/home/jovyan/{1}.tar.gz /Users/fangying/Downloads/{2}.tar.gz'.format(
                    name, name, name), shell=True, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            # print('kubectl exec -it -n jhub {} '
            #     '-- tar -zxf /home/jovyan/train_rubbish.tar.gz -C /home/jovyan/'.format(name))
            # subprocess.run(
            #     'kubectl exec -it -n jhub {} '
            #     '-- tar -zxf /home/jovyan/train_rubbish.tar.gz -C /home/jovyan/'.format(name),
            #     shell=True)
            # subprocess.run(
            #     ["kubectl", "exec", "-it", name, "-n", "jhub", "--", "wget", "-q","/home/jovyan/train_rubbish.tar.gz",
            #      "https://newlandblob.blob.core.chinacloudapi.cn/test/train_rubbish.tar.gz"])
            # subprocess.run(
            #     ["kubectl", "exec", "-it", name, "-n", "jhub", "--", "tar", "-zxf", "/home/jovyan/train_rubbish.tar.gz", "-C",
            #      "/home/jovyan/"])
            LOG.info("结束做实验%s" % name)
...
def main():
    ...
    try:
        if args.command == 'purge':
            ...
            experiment(args.prefix, args.start, args.end, args.cmd, args.workers)
