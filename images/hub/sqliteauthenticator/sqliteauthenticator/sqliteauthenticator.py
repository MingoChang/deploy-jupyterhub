...
class SQLiteAuthenticator(Authenticator):
    ...
    def sqlite_add_user(self,username,password):
        if self.admin_mode == True or True:
            encryptor = prpcrypt()
            sql_cnn = sqlite3.connect(os.getenv('JUPYTERHUB_SQLITEDB_PATH'))
            c = sql_cnn.cursor()
            input_password = encryptor.encrypt(password).decode()
            query = "INSERT INTO `users` (username, password) VALUES ('{}','{}') ".format(username,input_password)
            c.execute(query)
            sql_cnn.commit()
            c.close()
            sql_cnn.close()
            userpath = ('/home/{}').format(username)
            if not os.path.exists(userpath) and False: #in case add_user in jupyterhub failed
